...
type Helm interface {
	Deploy(kubeConfig, name, namespace, chartPath, valuesPath string, valuesString map[string]string) error
...
func (h *HelmImpl) Deploy(kubeConfig, name, namespace, chartPath, valuesPath string, valuesString map[string]string) error {
...
func (h *HelmImpl) Destroy(kubeConfig, name, namespace string) error {
	// TODO Need to be fix the context names
	s := kube.GetConfig(kubeConfig, "default", namespace)
	cfg := &action.Configuration{}
	err := cfg.Init(s, namespace, "", func(format string, v ...interface{}) {
